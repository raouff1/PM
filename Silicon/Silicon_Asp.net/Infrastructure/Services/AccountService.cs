...
public class AccountService(UserManager<UserEntity> userManager, DataContext context, IConfiguration configuration)
{
    ...
    public async Task<bool> UpdateBasicInfoAsync(ClaimsPrincipal claimsPrincipal, AcountBasicInfo basicInfo)
    {
        try
        {
            var nameIdentifier = claimsPrincipal.FindFirst(ClaimTypes.NameIdentifier)!.Value;
    ...
    public async Task<bool> UpdateAddressInfoAsync(ClaimsPrincipal claimsPrincipal, AcountAdressInfo addressInfo)
    {
        try
        {
            var nameIdentifier = claimsPrincipal.FindFirst(ClaimTypes.NameIdentifier)!.Value;
    ...
    public async Task<bool> UploadUserProfileImageAsync(ClaimsPrincipal userClaims, IFormFile file)
    {
        try
        {
            if (userClaims != null && file != null && file.Length != 0)
            {
                ...
                if (user != null)
                {
                    var fileName = $"p{user.Id}_{Guid.NewGuid()}{Path.GetExtension(file.FileName)}";
